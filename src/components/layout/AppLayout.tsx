...
import { LayoutDashboard, Swords, PlusCircle, Clock, Users, Banknote, Inbox, Menu, X } from 'lucide-react';
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-felt-900 border-b border-gold-400/10 sticky top-0 z-40">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-md bg-gold-400/15 border border-gold-400/25 flex items-center justify-center text-gold-400 font-display font-bold text-[10px]">CL</div>
            <span className="font-display text-sm font-semibold text-ivory-100">CribLedger</span>
          </div>
          <button onClick={() => setSidebarOpen(true)} className="text-ivory-200/55 hover:text-ivory-100 p-1">
            <Menu size={20} />
          </button>
        </header>

        <main className="flex-1 overflow-auto">
          <div className="max-w-4xl mx-auto px-4 py-6 pb-24 md:pb-8">{children}</div>
        </main>
      </div>
