...
export default function ActiveMatch() {
  ...
  const [backedPlayerId, setBackedPlayerId] = useState("");
  const [counterpartyId, setCounterpartyId] = useState("");
  const [proposerAmount, setProposerAmount] = useState("");
  const [counterpartyAmount, setCounterpartyAmount] = useState("");
  const [wagerNotes, setWagerNotes] = useState("");
  useEffect(() => {
    const params = new URLSearchParams(location.search);

    if (params.get("wager") === "true") {
  ...
  const matchPlayers = [playerA, playerB].filter(Boolean).map((u) => ({
    id: u!.id,
    name: u!.display_name,
  }));
  const winner = match.winner_id
    ? users.find((u) => u.id === match.winner_id)
    : null;
  const loser = winner
    ? users.find(
        (u) =>
  ...
  const pendingWagers = wagers.filter(
    (w) => w.status === "proposed" || w.status === "accepted",
  );
  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto">
          ...
          <CardBody>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-display font-semibold text-ivory-100 mb-0.5">
                  Match Lobby
                </p>
                <p className="text-xs text-ivory-200/50 font-body">
                  This match hasn't started yet. Side wagers can still be
                  proposed and accepted. Once started, terms lock and accepted
                  bets go live.
                </p>
                {wagers.length > 0 && (
                  <p className="text-xs text-amber-300/70 font-body mt-1">
                    {wagers.filter((w) => w.status === "proposed").length}{" "}
                    proposed ·{" "}
                    {wagers.filter((w) => w.status === "accepted").length}{" "}
                    accepted side wager{wagers.length !== 1 ? "s" : ""}
                  </p>
                )}
              </div>
            </div>
              ...
              {!confirmStart ? (
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-start gap-2 py-2 px-3 rounded-lg bg-amber-400/6 border border-amber-400/15 text-xs text-amber-300 font-body">
                    ...
                    <span>
                      This will lock the match wager and activate all accepted
                      side bets. Proposed bets not yet accepted will remain
                      proposed. This cannot be undone.
                    </span>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <Button
                      variant="secondary"
                      size="md"
                      className="flex-1 justify-center text-crimson-300 hover:text-crimson-200"
                      onClick={async () => {
                        await supabase
                          .from("matches")
                    ...
                    >
                      <Play size={13} />{" "}
                      {startMatch.isPending ? "Starting…" : "Confirm Start"}
                    </Button>
                  </div>
                </div>
              )}
              ...
              <React.Fragment key={playerId}>
                {i === 1 && (
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <span className="text-xs text-ivory-200/25 font-body">
                      vs
                    </span>
                  </div>
                )}
                <div
                  className={`flex flex-col items-center gap-2 transition-opacity ${
                    isCompleted && leadId && leadId !== playerId
                      ? "opacity-40"
                      : ""
                  }`}
                >
                  ...
                  {!isPending ? (
                    ...
                    <p className="text-2xl font-mono font-bold text-ivory-100/20">
                      —
                    </p>
                  )}
                </div>
      ...
      {isActive && (
        <Card className="mb-5">
          <CardHeader>
            <p className="text-sm font-display font-semibold text-ivory-100">
              Enter Final Scores
            </p>
          </CardHeader>
          <CardBody className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3">
              <Input
                label={`${playerA?.display_name ?? "Player A"} Score`}
                type="number"
                min="0"
                max="121"
                value={scoreAStr}
              ...
              />
              <Input
                label={`${playerB?.display_name ?? "Player B"} Score`}
                type="number"
                min="0"
                max="121"
                value={scoreBStr}
            ...
            {previewPayout !== null && (
              <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-gold-400/5 border border-gold-400/12">
                <span className="text-xs text-ivory-200/45 font-body">
                  Payout ({Math.abs(scoreA - scoreB)} pt diff) → obligation
                  created on finalize
                </span>
            ...
            {!confirmFinalize ? (
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-start gap-2 py-2.5 px-3 rounded-lg bg-gold-400/8 border border-gold-400/18 text-sm text-gold-300 font-body">
                  ...
                  <span>
                    This will settle all active side wagers and create
                    obligation records. Scores are final.
                  </span>
                </div>
                <div className="flex gap-2">
                  <Button
                  ...
                  >
                    <CheckCircle size={14} />{" "}
                    {finalizeMatch.isPending ? "Settling…" : "Confirm & Settle"}
                  </Button>
                </div>
              </div>
            )}
        ...
        <Card gold className="mb-5 animate-slide-up">
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-display font-semibold text-ivory-100">
                Obligation Created
              </span>
            </div>
          </CardHeader>
          <CardBody className="flex flex-col gap-3">
            <div className="flex items-center justify-between py-3 px-4 rounded-lg bg-gold-400/8 border border-gold-400/18">
              <div>
                <p className="text-xs text-ivory-200/45 font-body">
                  Debt record
                </p>
                <p className="text-sm text-ivory-100 font-body mt-0.5">
                  <strong>{loser.display_name}</strong> owes{" "}
            ...
            <p className="text-xs text-ivory-200/35 font-body">
              ...
              to record payment when money changes hands.
            </p>
          </CardBody>
        </Card>
      )}
            ...
            <p className="text-xs text-ivory-200/40 font-body mt-0.5">
              ...
              {isPending && pendingWagers.length > 0 && (
                <span className="ml-1 text-amber-400/70">
                  · {pendingWagers.length} open for acceptance
                </span>
              )}
            </p>
          </CardHeader>
          <CardBody className="pt-2 flex flex-col gap-2">
            {wagers.map((w) => {
              ...
              const counterparty = users.find(
                (u) => u.id === w.counterparty_id,
              );
              const backedPlayer = users.find(
                (u) => u.id === w.proposer_picks_player_id,
              );
              const oddsLabel = impliedOddsLabel(
                w.proposer_amount,
                w.counterparty_amount,
              );
              const wagerWinner = w.winner_id
                ? users.find((u) => u.id === w.winner_id)
                : null;
              const wagerLoser = w.loser_id
                ? users.find((u) => u.id === w.loser_id)
                : null;

              return (
                <div
                  key={w.id}
                  className="py-3 px-4 rounded-lg bg-felt-800/45 border border-felt-600/35"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-ivory-200/40 font-body mt-0.5">
                        vs {counterparty?.display_name} · ${w.proposer_amount}:$
                        {w.counterparty_amount} ({oddsLabel})
                      </p>
                      {wagerWinner && wagerLoser && (
                        <p className="text-xs font-body mt-1">
                          <span className="text-emerald-400">
                            {wagerWinner.display_name} won
                          </span>
                          <span className="text-ivory-200/35">
                            {" "}
                            — obligation created
                          </span>
                        </p>
                      ...
                      {isPending && w.status === "proposed" && (
                        <p className="text-xs text-amber-400/60 font-body mt-1">
                          Awaiting acceptance
                        </p>
                      )}
                    </div>
      ...
      {showWagerForm && (
        <Card className="mb-4 border border-gold-400/20">
          <CardBody>
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-ivory-200/40">
                  Contextual wager creation coming next
                </p>
              </div>
            ...
            <div className="rounded-lg border border-felt-600/30 bg-felt-800/30 p-4">
              <div className="grid grid-cols-2 gap-3">
                <Select
                  label="Proposer"
                  value={proposerId}
                ...
                <Select
                  label="Backs Player"
                  value={backedPlayerId}
                  onChange={(e) => setBackedPlayerId(e.target.value)}
                >
                  <option value="">Select…</option>
                ...
                <Input
                  label="Proposer Risks ($)"
                  type="number"
                  value={proposerAmount}
                  onChange={(e) => setProposerAmount(e.target.value)}
                  placeholder="10.00"
                />

                <Input
                  label="Counterparty Risks ($)"
                  type="number"
