...
import {
  useActiveMatches, usePendingMatches, useUsers, useStartMatch,
} from '@/hooks/useData';
import {
  Card, CardBody, Button, SectionTitle, SuitRow, EmptyState,
  PlayerAvatar, MatchStatusBadge, LoadingCard,
} from '@/components/ui';
import { PlusCircle, ChevronRight, Swords, Timer, Play, AlertCircle } from 'lucide-react';
...
function PendingMatchCard({ match, users }: { match: Match; users: { id: string; display_name: string; avatar_color: string; role: string; auth_id: string | null; created_at: string; updated_at: string; is_active: boolean }[] }) {
    ...
    <Card key={match.id} className="mb-3 border border-amber-400/15">
      <CardBody>
        <div className="flex items-center gap-4">
          ...
          {/* Match info */}
          <div className="flex-1 min-w-0" onClick={() => navigate(`/matches/${match.id}`)}>
            ...
            <div className="flex items-center gap-2 mt-0.5 text-xs text-ivory-200/40 font-body">
              ...
              <span>·</span>
              <span className="text-amber-400/70">{formatRelativeTime(match.created_at)}</span>
            </div>
          </div>
        ...
        {confirming && (
          <div className="mt-3 flex items-start gap-2 py-2 px-3 rounded-lg bg-amber-400/6 border border-amber-400/15 text-xs text-amber-300 font-body">
            ...
            <span>Starting the match will lock the wager terms and activate any accepted side bets. This cannot be undone.</span>
          </div>
        )}
      </CardBody>
    </Card>
...
export default function ActiveMatchesList() {
  const navigate = useNavigate();
  const { data: activeMatches  = [], isLoading: activeLoading  } = useActiveMatches();
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-3xl font-display font-bold text-ivory-100">Matches</h1>
          <p className="text-sm text-ivory-200/45 font-body mt-0.5">Pending lobby · active games</p>
        </div>
        <SuitRow />
      </div>
        ...
        <Card>
          <CardBody>
            <EmptyState
              ...
              description={pendingMatches.length > 0
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 text-xs text-ivory-200/40 font-body">
                    ...
                    <span>${match.winner_bonus} bonus</span>
                    {match.started_at && (
                      <><span>·</span><span className="text-emerald-400/60">started {formatRelativeTime(match.started_at)}</span></>
                    )}
                    {match.notes && <><span>·</span><span className="italic truncate max-w-[100px]">{match.notes}</span></>}
                  </div>
                </div>
