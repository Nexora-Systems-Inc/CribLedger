  ...
  return (
    <div className="animate-fade-in">
          ...
          >
            <CardBody className="py-3">
              <div className="flex items-center gap-1.5 mb-1">
                <span
                  className={`text-sm ${accent ? "text-emerald-400" : warn ? "text-crimson-400" : pending && value > 0 ? "text-amber-400" : "text-gold-400/50"}`}
                >
                  {icon}
                </span>
                <span className="text-xs text-ivory-200/45 font-body uppercase tracking-wider">
                  {label}
                </span>
              </div>
              <p
      ...
      {pendingMatches.length > 0 && (
        <Card
          className="mb-4 cursor-pointer border border-amber-400/20 bg-amber-400/4"
      ...
      {activeMatches.length > 0 && (
        <Card
          ...
          onClick={() => navigate(`/matches/${activeMatches[0].id}`)}
        >
          <CardBody className="py-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-body text-ivory-100">
                  <span className="font-semibold">
                    ...
                    {activeMatches.length > 1 ? "es" : ""}
                  </span>
                  <span className="text-ivory-200/55"> in progress</span>
                </span>
              </div>
              <span className="text-xs text-gold-400 font-body flex items-center gap-1">
      ...
      <div className="grid md:grid-cols-5 gap-5">
          ...
          <CardBody className="py-2 px-2">
            {balLoading ? (
              ...
              <EmptyState
                ...
                description="Complete a match to generate ledger entries"
              />
            ) : (
              (balances ?? []).map((ub, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-base font-display w-5 text-center shrink-0 ${i === 0 ? "text-gold-300" : i === 1 ? "text-ivory-200/70" : i === 2 ? "text-amber-700" : "text-ivory-200/30"}`}
                    >
                      {i < 3 ? rankIcons[i] : i + 1}
                    </span>
                    <PlayerAvatar user={user} size="sm" />
                    <span className="flex-1 text-sm font-body text-ivory-100">
                      {user.display_name}
                    </span>
        ...
        <div className="md:col-span-2 flex flex-col gap-4">
          <Card>
            <CardBody>
              <p className="text-xs text-ivory-200/45 font-body uppercase tracking-wider mb-3">
                Quick Actions
              </p>
              ...
              {outstanding.length === 0 ? (
                ...
                outstanding.slice(0, 4).map((ob) => {
                  ...
                  return (
                    <div
                      key={ob.id}
                      className="flex items-center justify-between py-2 px-3 rounded-lg bg-felt-800/50 border border-felt-600/30"
                    >
